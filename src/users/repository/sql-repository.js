...
const SQLRepository = () => {
  ...
  const get = (id, transaction=knex) =>
    transaction
      ...
      const getByloginData : (username: any, password: a... = (username : any, password : any)
: QueryBuilder<{}, DeferredKe ... => knex
.select(columnName: '*')
.from(tableName: 'users')
.where(object: { username, password })
.then(onfulfilled: handleNotFound(id: username))
.then(onfulfilled: decodeUser)
